...
16. Mobile typography and controls are scaled up for readability and touch use, including larger headings, labels, status text, action buttons, file rows, and editor text.
17. Mobile mode is triggered by a responsive rule that combines narrow viewport width (`max-width: 900px`) with a touch-device fallback (`max-width: 1024px` plus `hover: none` and `pointer: coarse`) so phones still get mobile behavior even when browsers report a wider layout viewport.
