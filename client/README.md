...
12. Files ending in `.md` open in a lightweight CodeMirror editor with Markdown syntax highlighting (for example, `*emphasis*` and `_emphasis_` render with styled text while the raw markers remain visible).
13. While the initial tree read is in flight, the client shows a loading state rather than incorrectly claiming the repo is empty.
14. The right pane is split into tabs:
...
15. New files are created from a basename-only prompt. The UI rejects path separators such as `/` and `\`.
16. On mobile-width layouts (including Samsung Galaxy S21+ viewport widths), the file tree stacks above the editor. Selecting a file scrolls down to the editor, and a `<` button in the editor header scrolls back to the file tree so another file can be chosen.
17. On mobile, the editor header is intentionally compact: it shows only the `<` back-to-files control, the selected filename (trimmed for small screens), and a small `Idle`/`Syncing` badge.
18. `Log out` is shown in the Files pane header instead of the editor header, and the editor-level `Sync now` action is removed.
19. Mobile typography and controls are scaled up for readability and touch use, while editor text itself is kept slightly smaller so more content fits on screen.
20. Mobile mode is triggered by a responsive rule that combines narrow viewport width (`max-width: 900px`) with a touch-device fallback (`max-width: 1024px` plus `hover: none` and `pointer: coarse`) so phones still get mobile behavior even when browsers report a wider layout viewport.
21. The client tracks the visible browser viewport height (`visualViewport.height` with `innerHeight` fallback) and applies it as a CSS variable so the editor pane fills the available window height, including mobile browser UI changes.
22. On mobile layouts, horizontal gutters are removed so the editor pane touches the screen edges and the editor text box spans the pane width with no left/right inset; rounded corners are preserved for both the pane and the text box.
23. The production build exposes a Web App Manifest and Service Worker, so Android Chrome can install the app from the browser menu and launch it in app/fullscreen display mode.
...
- Prefer plain-text editing defaults, while using lightweight editor extensions only where they materially improve readability (for example, Markdown syntax highlighting in `.md` files).
