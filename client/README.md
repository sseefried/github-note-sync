...
15. On mobile-width layouts (including Samsung Galaxy S21+ viewport widths), the file tree stacks above the editor. Selecting a file scrolls down to the editor, and a `<` button in the editor header scrolls back to the file tree so another file can be chosen.
16. On mobile, the editor header is intentionally compact: it shows only the `<` back-to-files control, the selected filename (trimmed for small screens), and a small `Idle`/`Syncing` badge.
17. `Log out` is shown in the Files pane header instead of the editor header, and the editor-level `Sync now` action is removed.
18. Mobile typography and controls are scaled up for readability and touch use, while editor text itself is kept slightly smaller so more content fits on screen.
19. Mobile mode is triggered by a responsive rule that combines narrow viewport width (`max-width: 900px`) with a touch-device fallback (`max-width: 1024px` plus `hover: none` and `pointer: coarse`) so phones still get mobile behavior even when browsers report a wider layout viewport.
20. The client tracks the visible browser viewport height (`visualViewport.height` with `innerHeight` fallback) and applies it as a CSS variable so the editor pane fills the available window height, including mobile browser UI changes.
21. On mobile layouts, horizontal gutters are removed so the editor pane touches the screen edges and the editor text box spans the pane width with no left/right inset; rounded corners are preserved for both the pane and the text box.
22. The production build exposes a Web App Manifest and Service Worker, so Android Chrome can install the app from the browser menu and launch it in app/fullscreen display mode.
