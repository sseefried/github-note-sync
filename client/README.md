...
11. Editor changes are written to IndexedDB first and queued locally per file. Normal text edits are replayed as diff-based `POST /api/ops` patch requests with `baseRevision` checks on debounce, blur, tab hide, periodic polling, and reconnect. If an older cached file has no known server revision yet, the client temporarily falls back to `PUT /api/file`, refetches that file to learn its revision, and then resumes patch-based sync.
...
- Prefer diff-based, idempotent patch ops for normal text edits, while keeping whole-file writes only as a guarded compatibility fallback.
