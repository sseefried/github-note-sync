...
13. When a patch op hits a revision conflict, the client does not blindly retry with another diff. Instead it stores the blocked op plus the original base text, shows a single `OK` action for that file, and after you press it sends whole-file conflict input to `POST /api/conflicts/commit-markers`. The server commits the merged result as ordinary text, with Git conflict markers only where needed.
