...
The current local-first write path has one normal sync protocol. Accepted edits are written to IndexedDB immediately, the client computes a range-replace patch from the last known server content to the current local content, and replay uses `POST /api/ops` with one idempotent patch op per file at a time. Once a file is dirty, the cached server base for that file is frozen; background reloads and conflict detection do not rewrite `serverContent` or `revision` underneath the local edit. Only acknowledged write success, explicit reload-from-server confirmation, successful conflict resolution, or explicit clean-file fast-forward confirmation advances that base. If an older in-flight write is acknowledged after newer local edits have already been saved, the client now advances that frozen server base without rewinding the newer local text, and it refuses to enqueue a replacement op if the file/op snapshot moved while that next op was being prepared. If a cached file does not have a valid `revision`, the client now fails closed and requires a refresh from the server before that file can sync again. Manual refresh, repo alias mutation, and tree-structure actions still call the server directly and therefore stay available only when the API is reachable.
