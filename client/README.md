...
19. On mobile, the editor header is intentionally compact: it shows the `<` back-to-files control, markdown preview toggle (for `.md` files), the selected filename in a slightly smaller heading that uses available space before truncating, and a small `Idle`/`Syncing` badge.
